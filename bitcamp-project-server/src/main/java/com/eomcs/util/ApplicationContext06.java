...
// 역할:
// - 클래스를 찾아 객체를 생성한다.
// - 객체가 일을 하는데 필요로 하는데 의존 객체를 주입한다.
// - 객체의 생성과 소멸을 관리한다.
//
public class ApplicationContext06 {
  ...
  private Object createInstance(Class<?> clazz) throws Exception {
    ...
    objPool.put(beanName == null ? clazz.getName() : beanName // 객체이름
        , obj // 객체
    );
    return obj;
  }
  ...
  private Object getParameterInstance(Parameter param) throws Exception {
    ...
    for (Object obj : objs) {
      ...
      if (param.getType().isInstance(objs)) {
        return obj;
      }
    }
    ...
    Class<?> clazz = findParmeterClassInfo(param.getType());
    if (clazz == null) {
  ...
  private Class<?> findParmeterClassInfo(Class<?> paramType) {
    // concrete class 목록에서 파라미터에 해당하는 클래스가 있는지 조사한다.
    for (Class<?> clazz : concreteClasses) {

      if (paramType.isInterface()) {
        // 파라미터가 인터페이스라면
        // 각각의 클래스에 대해 그인터페이스를 구현했는지 검사한다.
      ...
      // 파라미터 타입에 해당하는 타입이 concrete class 목록에 없다면,
      // 그냥 null 리턴한다.
    }
    return null;
  }
