  ...
  Runner createRunner(final String name) {
    ...
    class A implements Runner {
      ...
      public void run() {
        ...
        // => A인스턴스가 생성될 때 값을 따로 복제한 필드를 가리키는 것이다.
        //
        System.out.printf("%s님이 달립니다!", name);

        // 위 문장은 다음과 같이
        // 컴파일러가 로컬 클래스의 필드(예: parpamName)를 사용하는 문장으로 바꾼다.
