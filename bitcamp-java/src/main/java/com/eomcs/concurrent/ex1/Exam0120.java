...
// - 해결책:
//   CPU 시간을 배정받지 못할 때 마다 우선 순위를 높여서 
//   언젠가는 실행되게 만들게 했다.
...
//   - 메모리 낭비가 심하다.
//   - 복제된 프로세스는 독립적이기 때문에 실행 종료할때도 일일이 종료해야 한다.
//
// 2) 멀티 스레딩
// - 특정 코드만 분리하여 실행한다.
...
//     스레드도 자동 종료 된다. ( 그래서 관리가 편하다.)
// - 단점:
//   - 프로세스 복제 방식에 비해 코드 구현이 복잡하다.
//

// ## 컨텍스트 스위칭(Context Switching)
// - CPU를 실행 시간을 쪼개 이 코드 저 코드를 실행할때 마다 
//   실행 위치 및 정보(Context)를 저장하고 로딩하는 과정이 필요하다.
// - 이 과정을 'Context Switching' 이라 부른다.
// 
//
// ## 스레드(thread)
// - '실'이라는 뜻을 갖고 있다.
