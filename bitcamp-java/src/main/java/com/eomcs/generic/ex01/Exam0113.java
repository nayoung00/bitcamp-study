...
public class Exam0113 {
  ...
  static class Box<T> {
    ...
    // 그러나 보통 코드를 간락히하기 위해 한 개의 알파벳으로 표현한다. 
    ...
    // V - value
    // N - number
    // 여러개의 타입 파라미터를 선언해야 할 경우 보통 두 번째 파라미터 부터  다음 알파벳을 사용한다. 
    // S , U , V 등
    // 예) class Box<T, S, U, V> {...}
    //
    T value;  
  ...
  public static void main(String[] args) {
    ...
    // 클래스의 레퍼런스를 정의할 때 어떤 타입의 ㄱ밧을 다룰 것인지 제한을 걸어야 한다. 
    // 즉 지정해야 한다.
    Box<Member> box1;
    ...
    // 따라서 다음과 같이 값을 꺼낼 때도 형변환 할 필요가 없다.
    // Member m = box1.get();
