...
public class Exam0130 {
  ...
  public static void main(String[] args) {
    ...
    try {
      // Class.forName("fully-qualified class name(패키지 명을 포함한 클래스명)")
      // => 문자열 파라미터로 주어진 클래스를 찾아 메모리(Method Area)에 로딩한다.
      // => 이 과정에서 static {} 블록이 실행될 것이고,
      // 결국 Driver 인스턴스가 생성되어 DriverManager에게 등록도리 것이다.
      // => 확인!
      // 한 번 로딩된 클래스는 중복해서 로딩되지 않는다.
      // 따라서 static{} 블록도 중복해서 실행되지 않는다.
      // 그러니 Driver 인스턴스가 여러 개 생성될까 걱정하지 마라.
      // DriverManager에 같은 Driver 인스턴스가 여러 개 등록될까 걱정하지 마라.
      //
